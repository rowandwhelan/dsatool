...
combined();
  ...
  function combined(){
    ...
    document.getElementById("output").innerHTML += `<br><h3> Manifest Scanner: <span id="numberOutput">${Number(((avg2(manifest2) / avg(manifest)) * 0.2 * value1(manifest1)).toPrecision(3) )}</span></h3>`;
    document.getElementById("output").innerHTML += `<br><h3> BOM Scanner: <span id="numberOutput">${Number(((avg2(bom2) / avg(bom)) * 0.2 * value1(bom1)).toPrecision(3) )}</span></h3>`;
